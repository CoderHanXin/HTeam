...
export default {
  ...
  methods: {
    ...
    handleOk() {
      this.$refs.projectAddForm.validate(valid => {
        if (valid) {
          let project = {}
          project.name = this.project.name
          project.desc = this.project.desc
          projectService
            .add(this.teamId, project, this.checkedUsers)
            .then(res => {
              this.visable = false
              this.$refs.projectAddForm.resetFields()
              this.checkedUsers = []
              this.$emit('onProjectAddOk')
            })
        }
      })
    },
